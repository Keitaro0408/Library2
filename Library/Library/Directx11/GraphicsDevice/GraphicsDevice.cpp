...
namespace Lib
{
	namespace Dx11
	{
		...
		bool GraphicsDevice::Initialize(HWND _hWnd, RECT _windowSize)
		{
			...
			{
				MyOutputDebugString(TEXT("m_pDeviceDX11Managerクラスはすでに初期化されています"));
				return false;
			}
			...
			if (!InitDevice())
			{
				MyOutputDebugString(TEXT("InitDeviceに失敗しました"));
				return false;
			}

			if (!InitDisplay())
			{
				MyOutputDebugString(TEXT("InitDisplayに失敗しました"));
				ReleaseDevice();
				return false;
			}
			MyOutputDebugString(TEXT("DX11Managerの初期化に成功\n"));
