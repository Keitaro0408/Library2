...
namespace Lib
{
	...
	HRESULT Window::DispWindow(INT _width, INT _height, LPCTSTR _windowName)
	{
	...
	HRESULT Window::DispWindow(INT _width, INT _height, LPCTSTR _windowName, LPCTSTR _iconName)
	{
	...
	void Window::ChangeWindowSize(int _width, int _hight)
	{
		RECT rw, rc;
		GetWindowRect(m_hWnd, &rw); // ウィンドウ全体のサイズ
		GetClientRect(m_hWnd, &rc); // クライアント領域のサイズ
