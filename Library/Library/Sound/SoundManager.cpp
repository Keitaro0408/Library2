...
bool Lib::SoundManager::Initialize(HWND _hWnd)
{
	if (m_pDSound8 != nullptr)
	{
		MessageBox(_hWnd, TEXT("SoundManagerはすでに初期化されています"), TEXT("エラー"), MB_ICONSTOP);
		return false;
	}
...
void Lib::SoundManager::Finalize()
{
	if (m_pDSound8 != nullptr)
	{
		m_pDSound8->Release();
...
void Lib::SoundManager::SoundOperation(int _index, SOUND_OPERATION _operation)
{
	switch (_operation)
	{
	case SOUND_PLAY:
...
void Lib::SoundManager::SetSoundVolume(int _index, int _ratio)
{
	if (_ratio <= 0)
	{
...
bool Lib::SoundManager::LoadSound(LPSTR _pFileName, int* _pIndex)
{
...
void Lib::SoundManager::ReleaseSound(int _index)
{
...
bool Lib::SoundManager::ReadWave(LPSTR _pFileName, WAVEFORMATEX* _pWaveFormat, BYTE** _pWaveData, DWORD* _pWaveSize)
{
